...
node{
  stage('Deploy to cluster'){
    ...
    sh("./kubectl config set-cluster internal1 --server=https://${KUBERNETES_SERVICE_HOST}:${KUBERNETES_SERVICE_PORT_HTTPS} --certificate-authority=/var/run/secrets/kubernetes.io/serviceaccount/ca.crt")
